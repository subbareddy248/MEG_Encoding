...
def compute_diff_sig(sub, feat1, feat2):
    ...
    for i in range(4):
        p = [
            preds_feat1[i][j : j + blocksize]
            for j in range(0, len(preds_feat1[i]), blocksize)
        ]
    ...
    for i in range(4):
        p = [
            preds_feat2[i][j : j + blocksize]
            for j in range(0, len(preds_feat2[i]), blocksize)
        ]
    ...
    for s in tqdm(range(iters)):
        ...
        shuffled_r2_feat1 = r2_score(
            new_reals, new_preds_feat1, multioutput="raw_values"
        )
        shuffled_r2_feat1[real_r2_feat1 == 0] = 0

        shuffled_r2_feat2 = r2_score(
            new_reals, new_preds_feat2, multioutput="raw_values"
        )
