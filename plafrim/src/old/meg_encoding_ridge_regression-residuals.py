...
parser.add_argument(
    "-i", "--id", type=str, help="Index (sub=id%8+1, feat=id//8) of job"
)
...
def eval(feature_name, feature_file, sub, save_regressed_y=False):
    ...
    print(f"Subject: {sub}, Feature: {feature_name}")
    np.random.seed(9)
    kf = KFold(n_splits=4)
    ...
    for sub in np.arange(0, 1):
        ...
        for train, test in kf.split(np.arange(4)):
            ...
            np.save(
                os.path.join(save_dir, "{}_y_pred_{}".format(str(sub), split_num)),
                np.nan_to_num(y_pred),
            )
            np.save(
                os.path.join(save_dir, "{}_y_test_{}".format(str(sub), split_num)),
                y_test,
            )
            # np.save(os.path.join(save_dir, "{}_weights_{}".format(str(sub), split_num)),weights)
            np.save(
                os.path.join(save_dir, "{}_lbda_{}".format(str(sub), split_num)), lbda
            )
            ...
            if save_regressed_y:
                ...
                np.save(
                    os.path.join(
                        save_dir, "{}_y_regress_test_{}".format(str(sub), split_num)
                    ),
                    y_test - y_pred,
                )
                np.save(
                    os.path.join(
                        save_dir, "{}_y_regress_train_{}".format(str(sub), split_num)
                    ),
...
if __name__ == "__main__":
    allfeatures = [
        "bert-base-lw-res_5_1.npy",
        "bert-base-lw-res_5_2.npy",
        "bert-base-lw-res_5_3.npy",
        "bert-base-lw-rh-res_5_1.npy",
        "bert-base-lw-rh-res_5_2.npy",
        "bert-base-lw-rh-res_5_3.npy",
    ]
