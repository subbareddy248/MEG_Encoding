...
parser.add_argument("--subject", "-s", type=int, help="Subject id.", default="12")
parser.add_argument("--root", type=str, help="Data root directory.", default="./data/MEG-MASC")
parser.add_argument("--xdata", "-x", type=str, help="Formatted data directory.")
parser.add_argument("--ydata", "-y", type=str, help="Formatted targets directory.")
parser.add_argument("--method", "-m", type=str, help="RNN decoding method (seq2seq or seq2vec)")


def train_phoneme_decoder(X_train, X_test, y_train, y_test, method, params):
    if method == "seq2seq":
        model = MEG2phoneme_seq(params)
        warmup = round(np.abs(np.subtract(*params.epochs.baseline)) * params.sfreq)
    if method == "seq2vec":
...
if __name__ == "__main__":
    ...
    if args.xdata is None or args.ydata is None:
        X, y = as_task_data(args.subject, "MEG->phoneme", arg.root, P)
